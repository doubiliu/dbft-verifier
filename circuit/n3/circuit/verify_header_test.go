...
func TestVerifyHeader(t *testing.T) {
	...
	hashData := hash.NetSha256(network, current)
	...
	parentHash := uints.NewU8Array(parent.Hash().BytesBE())
	currentHash := uints.NewU8Array(current.Hash().BytesBE())

	circuit := VerifyHeaderWrapper{
		Parent:       pparent,
		Current:      pcurrent,
		PubsPoint:    pubPoints,
	...
	fmt.Println("Verify Header Time: ", time.Since(start))
